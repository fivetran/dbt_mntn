...
with base as (
    ...
    select *
    from {{ ref('stg_mntn__analytics_by_country_tmp') }}
    ...
    select
        {{
            fivetran_utils.fill_staging_columns(
                source_columns=adapter.get_columns_in_relation(ref('stg_mntn__analytics_by_country_tmp')),
                staging_columns=get_analytics_by_country_columns()
            )
        }}
...
final as (
    ...
    select
        ...
        {{ fivetran_utils.fill_pass_through_columns('mntn__country_passthrough_metrics') }}
    from fields
